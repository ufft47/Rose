...
void tformula_debugger::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	BOOST_FOREACH (const game_logic::debug_info &i, fdb_.get_call_stack()) {
		...
		stack_text << "#" << tintegrate::generate_format(i.counter(), "green")
				<<": \""<< tintegrate::generate_format(i.name(), "green")
				<< "\": '" << i.str() << "' " << std::endl;
		++c;
	}
	...
	BOOST_FOREACH (const game_logic::debug_info &i, fdb_.get_execution_trace()) {
		...
		if(!i.evaluated()) {
			execution_text << "#" << tintegrate::generate_format(i.counter(), "green")
					<< ": \"" << tintegrate::generate_format(i.name(), "green")
					<< "\": '" << i.str() << "' " << std::endl;
		} else {
			execution_text << "#" << tintegrate::generate_format(i.counter(), "blue")
					<< ": \"" << tintegrate::generate_format(i.name(), "blue")
					<< "\": '" << i.str() << "' = "
					<< tintegrate::generate_format(i.value().to_debug_string(NULL,false), "red")
					<< std::endl;
		}
	}
