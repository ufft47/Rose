...
ttext_box::ttext_box(bool multi_line, bool password)
	: tcontrol(COUNT)
...
void ttext_box::insert_str(const std::string& str)
{
	...
	if (password_) {
		...
		for (; it != end && len < max_password_len; ++ it) {
			...
			if (tintegrate::is_require_escape_unicode(ch)) {
				str2.append("\\");
			}
			len ++;
		}
...
void ttext_box::calculate_integrate(const int maximum_width)
{
	...
	if (maximum_width > 0) {
		...
		integrate_ = new tintegrate(integrate_label, maximum_width, -1, get_text_font_size(), uint32_to_color(color), text_editable_);
