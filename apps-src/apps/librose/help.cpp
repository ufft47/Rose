		...
		const unsigned width, int style)
{
	...
	try {
		...
		int attention_min_chars = width / (font_size * 2 / 3);
		if (attention_min_chars <= 0) {
			attention_min_chars = 15;
		} else if (attention_min_chars > 20) {
			attention_min_chars = 20;
		}
		const std::string& first_line = font::word_wrap_text(s, font_size, width, -1, 1, attention_min_chars);
