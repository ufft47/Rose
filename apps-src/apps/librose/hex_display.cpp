...
const map_location hex_display::map_2_loc(int x, int y) const
{
	...
	if(nearest_hex != NULL) {
		...
		const int centerx = (loc_2_screen_x(res) - main_map_view().x + xpos_) + hex_size()/2 - hex_width();
		const int centery = (loc_2_screen_y(res) - main_map_view().y + ypos_) + hex_size()/2 - hex_size();
...
{
	return static_cast<int>(main_map_view().x + (loc.x + border_.size) * hex_width() - xpos_);
}

int hex_display::loc_2_screen_y(const map_location& loc) const
{
	return static_cast<int>(main_map_view().y + (loc.y + border_.size) * zoom_ - ypos_ + (is_odd(loc.x) ? zoom_/2 : 0));
}
