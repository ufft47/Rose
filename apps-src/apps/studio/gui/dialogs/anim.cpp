...
void tparticular::update_to_ui_particular_edit(twindow& window, const std::string& frame_tag) const
{
	...
	text_box->set_value(str_cast(starting_frame_time_));

	find_widget<tcontrol>(&window, "cycles", false).set_active(cycles_);
	find_widget<ttext_box>(&window, "x", false).set_value(parameters_.x_.get_original());
	find_widget<ttext_box>(&window, "y", false).set_value(parameters_.y_.get_original());
	find_widget<ttext_box>(&window, "offset_x", false).set_value(parameters_.offset_x_.get_original());
	find_widget<ttext_box>(&window, "offset_y", false).set_value(parameters_.offset_y_.get_original());

	find_widget<ttext_box>(&window, "layer", false).set_value(str_cast(layer_));
...
void tparticular::update_to_ui_frame_edit(twindow& window, int n) const
{
	...
	find_widget<ttext_box>(&window, "duration", false, true)->set_value(str_cast(builder.duration_));

	find_widget<ttext_box>(&window, "image", false, true)->set_value(builder.image_);
	find_widget<ttext_box>(&window, "image_mod", false, true)->set_value(builder.image_mod_);
	find_widget<ttext_box>(&window, "image_diagonal", false, true)->set_value(builder.image_diagonal_.get_filename());
	find_widget<ttext_box>(&window, "image_horizontal", false, true)->set_value(builder.image_horizontal_.get_filename());
	find_widget<ttext_box>(&window, "x", false, true)->set_value(builder.x_.get_original());
	find_widget<ttext_box>(&window, "y", false, true)->set_value(builder.y_.get_original());
	find_widget<ttext_box>(&window, "halo", false, true)->set_value(builder.halo_.get_original());
	find_widget<ttext_box>(&window, "offset_x", false, true)->set_value(builder.offset_x_.get_original());
	find_widget<ttext_box>(&window, "offset_y", false, true)->set_value(builder.offset_y_.get_original());
	...
	find_widget<ttext_box>(&window, "sound", false, true)->set_value(builder.sound_.c_str());

	find_widget<ttext_box>(&window, "stext", false, true)->set_value(builder.stext_.c_str());
	find_widget<ttext_box>(&window, "font_size", false, true)->set_value(str_cast(builder.font_size_));

	find_widget<ttext_box>(&window, "text_color", false, true)->set_value(encode_color(builder.text_color_));
	find_widget<ttext_box>(&window, "alpha", false, true)->set_value(builder.highlight_ratio_.get_original());
}
...
bool tanim2::from_ui_particular_edit(twindow& window, int n)
{
	...
	int start_time = utils::to_int(text_box->get_value());
	ss.str("");
	...
	std::string text = find_widget<ttext_box>(&window, "x", false, true)->get_value();
	ss.str("");
	...
	text = find_widget<ttext_box>(&window, "y", false, true)->get_value();
	ss.str("");
	...
	text = find_widget<ttext_box>(&window, "offset_x", false, true)->get_value();
	ss.str("");
	...
	text = find_widget<ttext_box>(&window, "offset_y", false, true)->get_value();
	ss.str("");
	...
	int layer = utils::to_int(text_box->get_value());
	ss.str("");
...
{
	config cfg;
	cfg["duration"] = utils::to_int(find_widget<ttext_box>(&window, "duration", false, true)->get_value());
	
	std::string text;
	if (use_image) {
		text = find_widget<ttext_box>(&window, "image", false, true)->get_value();
		if (!text.empty()) {
			cfg["image"] = text;
		}
		text = find_widget<ttext_box>(&window, "image_diagonal", false, true)->get_value();
		if (!text.empty()) {
			cfg["image_diagonal"] = text;
		}
		text = find_widget<ttext_box>(&window, "image_horizontal", false, true)->get_value();
		if (!text.empty()) {
			cfg["image_horizontal"] = text;
		}
	} else {
		text = find_widget<ttext_box>(&window, "halo", false, true)->get_value();
		if (!text.empty()) {
			cfg["halo"] = text;
		}
	}
	text = find_widget<ttext_box>(&window, "image_mod", false, true)->get_value();
	if (!text.empty()) {
		cfg["image_mod"] = text;
	}
	text = find_widget<ttext_box>(&window, "x", false, true)->get_value();
	if (!text.empty()) {
		cfg["x"] = text;
	}
	text = find_widget<ttext_box>(&window, "y", false, true)->get_value();
	...
	text = find_widget<ttext_box>(&window, "sound", false, true)->get_value();
	if (!text.empty()) {
		cfg["sound"] = text;
	}
	text = find_widget<ttext_box>(&window, "stext", false, true)->get_value();
	const std::string stext = text;
	if (!stext.empty()) {
		cfg["stext"] = text;
	}
	int font_size = utils::to_int(find_widget<ttext_box>(&window, "font_size", false, true)->get_value());
	if (font_size) {
		cfg["font_size"] = font_size;
	}

	if (!stext.empty()) {
		int color = decode_color(find_widget<ttext_box>(&window, "text_color", false, true)->get_value());
	...
	text = find_widget<ttext_box>(&window, "alpha", false, true)->get_value();
	if (!text.empty()) {
		cfg["alpha"] = text;
	}
...
tanim3::tanim3(display& disp, const config& app_cfg)
	: disp_(disp)
	, app_cfg_(app_cfg)
	, app_anim_start_(twidget::npos)
	, clicked_anim_(twidget::npos)
{
	prepare_anims();
}
...
void tanim3::prepare_anims()
{
	...
	reserved_proj.insert("editor");
	reserved_proj.insert("kingdomd");
	reserved_proj.insert("librose");
	reserved_proj.insert("lualib");
	std::map<std::string, std::string> apps_in_apps_sln = apps_sln::apps_in(game_config::apps_src_path + "/apps");
	for (std::map<std::string, std::string>::const_iterator it = apps_in_apps_sln.begin(); it != apps_in_apps_sln.end(); ++ it) {
		const std::string& proj = it->first;
...
void tanim3::did_app_changed(ttoggle_button& widget)
{
	...
	for (std::vector<tanim2>::const_iterator it = anims_updating_.begin(); it != anims_updating_.end(); ++ it, anim_at ++) {
		...
		if (app_anim_start_ == twidget::npos) {
...
void tanim3::do_edit_animation()
{
	...
	tlistbox* list = find_widget<tlistbox>(window_, "animations", false, true);
	ttoggle_panel* cursel = list->cursel();
	list->set_row_child_label(cursel->at(), "id", anim.id_);
}
