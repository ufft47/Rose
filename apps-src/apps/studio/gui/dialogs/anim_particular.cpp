...
void tanim_particular::pre_show(CVideo& video, twindow& window)
{
	find_widget<tlabel>(&window, "title", false).set_label(_("anim^Edit particular"));

	if (at_ == 0) {
		anim_.unit_anim_.update_to_ui_particular_edit(window, "frame");
	} else {
		std::map<std::string, tparticular>::iterator it = anim_.sub_anims_.begin();
		std::advance(it, at_ - 1);
		it->second.update_to_ui_particular_edit(window, it->first);
